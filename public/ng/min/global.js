...
var app = angular.module("comedian", [ "ngRoute" ]).config([ "$routeProvider", "$locationProvider", function($routeProvider, $locationProvider) {
    angular.forEach([ {
        url: "/",
...
(function(w) {
    var objectTypes = [ "String", "Number", "Undefined", "Object", "Function", "Boolean" ], callback = function(type) {
...
(function(w, d) {
    ...
    w.load = function(file, callback) {
        if (!isString(file)) for (var i = 0; i < file.length; i++) load(file[i], callback); else if (_plugins[file]) load(_plugins[file], callback); else file.split(".").pop() === "js" ? loadJS(file, callback) : loadCSS(file, callback);
    };
